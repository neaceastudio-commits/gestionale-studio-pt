...
function aggiornaKPI() {
  const clienti = clientiGestibiliInCentrale();
  const tot  = clienti.length;
  const att  = clienti.filter(c => c.statoAbbonamento === 'Attivo').length;
  const sch  = clienti.filter(c => leggiStorage(c.id, 'schede')?.length).length;
  const incomp = clienti.filter(c => !leggiStorage(c.id, 'datiFisici')?.length).length;
  ...
  document.getElementById('lista-sub').textContent  = accessoCentralePt.attivo
    ? tot + ' clienti assegnati'
    : tot + ' clienti totali';
}
...
function renderLista() {
  ...
  let lista = clientiGestibiliInCentrale().filter(c => {
  ...
  body.innerHTML = lista.map(c => {
    ...
    const impBtn = accessoCentralePt.attivo
      ? ''
