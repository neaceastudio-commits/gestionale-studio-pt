...
Cruscotto e Centrale usano `studio-audit-access.js` e il gateway centrale `studio-calendar-activity`, riservato a Direzione verificata. Accesso via sessione oppure email/codice già emesso dal Portale PT. Nessun fallback REST; nessuna nuova chiave nei due siti statici. Le due pagine conservano gli scrittori production censiti e l’agenda di sola consultazione. Sviluppi compensi, motore di calcolo, tariffe locali e relativa UI sono esclusi dal candidato audit. `saveClientAdmin`, `savePayment`, `quickPay` inviano esclusivamente aggiornamenti cliente al gateway.

Nessun backfill o trasferimento automatico dello storico: Gianluca lo trasferirà manualmente. Le migrazioni definiscono schema e funzioni senza popolare lo storico; il gateway registra solo operazioni esplicite successive. La sincronizzazione di una singola assegnazione è parte della modifica cliente richiesta, non un trasferimento massivo.
