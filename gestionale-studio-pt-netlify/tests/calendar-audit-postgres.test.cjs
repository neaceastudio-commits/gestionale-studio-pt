...
(async()=>{const db=await start();const c=db.client;try{
 ...
 const availabilityBefore=(await c.query('select * from operator_availability order by operator_id,day_key')).rows;
 ...
 assert.deepEqual((await c.query('select * from operator_availability order by operator_id,day_key')).rows,availabilityBefore);
 ...
 const changedLog=(await read('owner'))[0];assert.equal(changedLog.actor_operator_id,'pt');assert.equal(changedLog.actor_email,'pt@example.test');assert.equal(changedLog.source,'calendar');
 const availabilityAfter=(await c.query('select * from operator_availability order by operator_id,day_key')).rows;assert.equal(availabilityAfter.filter((r,i)=>JSON.stringify(r)!==JSON.stringify(availabilityBefore[i])).length,1);
 const logsBeforeNoop=(await read('owner')).length;await write('availability',{rows:[{operator_id:'pt',day_key:'tue',slots:[' 18:00-19:00 ','18:00-19:00']}]});assert.equal((await read('owner')).length,logsBeforeNoop);assert.deepEqual((await c.query('select * from operator_availability order by operator_id,day_key')).rows,availabilityAfter);
