...
export default async request=>{
 ...
 try{
  ...
  phase='calendar verification';const cal=new core.CalDAV(process.env);await cal.verify();const href='neacea-'+crypto.createHash('sha256').update(b.id).digest('hex')+'.ics';phase='event read';const a=await cal.read(href);
  if(b.operation==='read'){
 ...
 }catch{return Response.json({error:'Controlled Apple probe refused at '+phase},{status:409})}
};
