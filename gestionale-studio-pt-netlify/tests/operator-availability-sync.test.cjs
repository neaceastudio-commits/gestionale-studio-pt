...
async function setup(local={},rows=fixture()){
 ...
 const c=vm.createContext({window:{},URLSearchParams,location:{search:'?access=SIM_VERIFIED'},sessionStorage:{getItem(){return null}},UI:{showToast(){}},console:{warn(){}},setTimeout(){},document:{addEventListener:(n,f)=>{if(n==='DOMContentLoaded')ready=f},getElementById:id=>id==='pt-staff-save-result'?message:null,querySelectorAll:selector=>selector==='[data-pt-staff-day]'?editor:[]},localStorage:{getItem:k=>cache.get(k)||null,setItem:(k,v)=>cache.set(k,v)},fetch:async(url,options)=>{
  const u=new URL(url,'https://calendar.test');let body=options.body?JSON.parse(options.body):null;
  if(options.method==='POST'){assert.equal(u.pathname,'/.netlify/functions/calendar-activity');assert.equal(body.accessToken,'SIM_VERIFIED');assert.equal(body.operation,'availability');assert.equal(body.actor_name,undefined);body=body.payload.rows;}else{assert.equal(u.pathname,'/rest/v1/operator_availability');}
  calls.push({method:options.method,body});
  if((options.method==='GET'&&failRead)||(options.method==='POST'&&failPost))return {ok:false,status:503,text:async()=>'SIMULATED_FAILURE',json:async()=>({error:'SIMULATED_FAILURE'})};
  if(options.method==='POST'){for(const r of body){const old=remote.find(x=>x.operator_id===r.operator_id&&x.day_key===r.day_key);if(old)Object.assign(old,r);else remote.push(r)}return {ok:true,status:200,json:async()=>({success:true})}}
  assert.equal(options.method,'GET');return {ok:true,status:200,text:async()=>JSON.stringify(remote)};
 }});
 vm.runInContext(fs.readFileSync(path.join(root,'calendar-audit.js'),'utf8'),c);
 ...
 return {open,edit,calls,remote,cache,message,disableGateway:()=>{delete c.window.CalendarAudit},save:()=>c.window.PTAvailabilityOverview.saveStaffAvailability(),push:data=>c.SupabaseSync.pushOperatorAvailability(data),failRead:value=>failRead=value,failPost:value=>failPost=value};
}
...
test('missing audit gateway never falls back to a direct availability write',async()=>{const app=await setup();await app.open();app.disableGateway();const result=await app.push({SIM_0:{mon:{slots:['18:00-19:00']}}});assert.match(result.error,/Gateway audit/);assert.equal(app.calls.filter(c=>c.method==='POST').length,0);});
