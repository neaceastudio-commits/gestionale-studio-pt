...
function service({env=process.env,db=auth.db,store,cal=new CalDAV(env),now=Date.now}={}){
 ...
 async function provision(id,check){
  const k=key(id);let entry=await store.getWithMetadata(k,{type:'json'});if(entry?.data.stage==='linked')return;
  let n=await read(id);need(Date.parse(n.created_at)>=Date.parse(env.APPLE_CALDAV_START_AT)&&n.date>=env.APPLE_CALDAV_START_AT.slice(0,10),'Historical backfill forbidden');need(n.status==='prenotato','Only booked appointments can be linked');
  if(!entry){const record={id,href:'neacea-'+crypto.createHash('sha256').update(id).digest('hex')+'.ics',uid:id+'@calendar.neacea.it',marker:crypto.randomUUID(),calendar:env.APPLE_CALDAV_URL,stage:'pending',baseline:{slot:slot(n),guard:guard(n),apple:slot(n)}};entry={data:record,etag:await save(k,record)}}
 ...
 async function one(entry,check){const record=entry.data;need(record.calendar===env.APPLE_CALDAV_URL,'Collection changed');if(record.stage==='pending'){await provision(record.id,check);return 'linked'}
