...
exports.authorize=async(token,{env=process.env,db=auth.db}={})=>{
 const signed=auth.verify(token,env.APPLE_CALDAV_CALENDAR_SECRET);
 if(!env.APPLE_CALDAV_CALENDAR_SECRET||signed?.operatorId!=='staff_1'||signed.accessLevel!=='owner')return false;
 const rows=await db('operator_effective_roles',{query:'?select=*&operator_id=eq.staff_1&active=eq.true'}),op=rows[0];
 return !!op&&String(op.email).toLowerCase()===String(signed.email).toLowerCase()&&[...(op.system_roles||[]),...(op.legacy_roles||[])].some(r=>['owner','admin','direzione','titolare'].includes(String(r).toLowerCase()));
};
exports.handle=async(request,{env=process.env,db=auth.db,service}={})=>{
 ...
 try{
  const b=await request.json();if(!await exports.authorize(b.accessToken,{env,db}))return reply(403,{error:'Sessione Direzione richiesta'});
